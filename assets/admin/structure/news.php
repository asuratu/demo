...
//                                { name:'置顶', dbAction: { action : this.handlerUrl + 'SetFieldValue', field : 'is_status', fieldValue : 3 }, onpress:this.setFieldValue },
                                ...
                                { name:'是否显示', dbAction: { action : this.handlerUrl + 'SetFieldStatus', field : 'is_display' }, onpress:this.setFieldStatus },
                                {separator:true},
                                { name:'是否首显', dbAction: { action : this.handlerUrl + 'SetFieldStatus', field : 'is_home_display' }, onpress:this.setFieldStatus },
                                {separator:true},
                            ...
                            colModel : [
                                ...
                                {display:'新闻标题',  width:500, name:'a.title', align:'left'},
                                {display:'新闻列表图片', width:150, align:'center'},
                                {display:'是否显示', name:'a.is_display', width:50, align:'center'},
                                {display:'是否首显', name:'a.is_home_display', width:50, align:'center'},
                    ...
                    UPLOADSWF_LOAD_FLAG:false  //规定只加载一次，防止多次加载冲突。用于多图上传。
                }
            }();
    ...
    <div class="grid_wrapper">
        <div class="m_search_field">
            <form name="search_from" onsubmit="return false;">
                ...
                <?= App\Admin\Helper::getDefaultSearchInfo1($this->__LANGUAGE_CONFIG__) ?>
                <?= $this->sltIDTree ?>
                <input type="submit" class="ipt_btn" value="搜索" />
            </form>
        </div>
    </div>
