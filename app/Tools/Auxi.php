        ...
        if (in_array($_sp . '.' . $action, (array) $power, true)) {
            switch (strtolower($action)) {
                ...
                case 'delete':
                    ...
                    break;
                case 'export': //导出
                    $_r = $isDefined ? $title : "{name:'$title', onpress:this.exportExcel},{separator:true},";
                    break;
                case 'batch': //批量处理
                    $_r = $isDefined ? $title : "{name:'$title', onpress:this.batch},{separator:true},";
                    break;
                default:
                    $_r = $title;
                    break;
...
//                    $_r = $isDefined ? $title : "{name:'添加{$title}', bclass:'add', onpress:this.add},{separator:true},";
    ...
    public static function getPages($rows, $uri, $page = 1, $num = 10, $isHtmlPage = 0, $control = false) {
        ...
        if ($rows && $rows > $num) {
            ...
            if ($page <= 1) {
                $_startLink = '[首页]';
                $_prevLink = '<a href="javascript:;">上一页</a>';
            } else {
                $_prev = $page - 1;
                $_startLink = '<a href="' . $uri . $href . '">首页</a>';
                $_prevLink = '<a href="'
                    . ($_prev == 1 ? $uri  :
                        ($uri . '/' . $_prev . $href)
                    )
            ...
            if ($page >= $_pages) {
                $_nextLink = '<a href="javascript:;">下一页</a>';
                $_endLink = '[尾页]';
            } else {
                $_next = $page + 1;
                $_nextLink = '<a href="'
            ...
            for ($i = $_b; $i <= $_e; $i++) {
                ...
                if ($i != $page)
                    ...
                    $_numLink .= ' <a class="active" href="javascript:;">'.$i.'</a>';
            }
            ...
            return '<div class="page">' . $_prevLink . ' ' . $_numLink . ' '
            . $_nextLink . '</div>';
        }
    }

    public static function getEnPages($rows, $uri, $page = 1, $num = 10, $isHtmlPage = 0, $control = false) {
        ...
        if ($rows && $rows > $num) {
            ...
            if ($page <= 1) {
                $_startLink = '[首页]';
                $_prevLink = '<a href="javascript:;">Last Page</a>';
            } else {
                $_prev = $page - 1;
                $_startLink = '<a href="' . $uri . $href . '">First Page</a>';
                $_prevLink = '<a href="'
                    . ($_prev == 1 ? $uri  :
                        ($uri . '/' . $_prev . $href)
                    )
            ...
            if ($page >= $_pages) {
                $_nextLink = '<a href="javascript:;">Next Page</a>';
                $_endLink = '[Last Page]';
            } else {
                $_next = $page + 1;
                $_nextLink = '<a href="'
