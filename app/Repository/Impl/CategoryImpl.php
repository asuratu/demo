...
class CategoryImpl implements Category {
    ...
    public function find($categoryId, $languageId) {
        ...
        return $this->db->select('c.*, cs.*, c2.`category_name` as root_name, c2.`category_name_english` as root_english_name')
            ->table('`#@__@category` c, `#@__@category_substance` cs, `#@__@category` c2')
            ->where("c.`{$_field}` = ? AND c.`category_id` = cs.`category_id` AND c.`language` = ? AND c.`root_id` = c2.`category_id`")
