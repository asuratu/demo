...
class ArchivesImpl implements Archives {
    ...
    public function getCategoryRs($categoryId, $usePagination = false) {
        ...
        if ($this->currentCategoryRs) {
            ...
            $aryCategory['level'] = $this->currentCategoryRs->level;
            ...
            $aryCategory['rootName'] = $this->currentCategoryRs->root_name;
            $aryCategory['listDir'] = $this->currentCategoryRs->list_dir;
            $aryCategory['rootEnglishName'] = $this->currentCategoryRs->root_english_name;
            ...
            $aryCategory['getNoaBreadcrumb'] = $this->getNoaBreadcrumb($aryCategory['idTree']);
            $aryCategory['getEnNoaBreadcrumb'] = $this->getEnNoaBreadcrumb($aryCategory['idTree']);
    ...
    public function getSubstance($id) {
        ...
        if ($this->currentContentRs) {
            ...
            $arySubstance['synopsis'] = $this->currentContentRs->synopsis;
            $arySubstance['file_id'] = $this->currentContentRs->file_id;
            $arySubstance['archives_id_str'] = $this->currentContentRs->archives_id_str;
            $arySubstance['categoryId'] = $this->currentContentRs->category_id;
            $arySubstance['title'] = $this->currentContentRs->title;
            $arySubstance['cover'] = $this->currentContentRs->cover;
            ...
            $arySubstance['getNoaBreadcrumb'] = $this->getNoaBreadcrumb($arySubstance['idTree']);

            $arySubstance['getTagsList'] = $this->getTagsList($arySubstance['currentArchivesId']);
            $arySubstance['getPrevNext'] = $this->getPrevNext($this->currentContentRs->sort, $arySubstance['categoryId']);
            $arySubstance['getEnPrevNext'] = $this->getEnPrevNext($this->currentContentRs->sort, $arySubstance['categoryId']);
    ...
    public function getNoaBreadcrumb($idTree) {
        if (!$idTree) {
            return;
        }
        ...
        foreach ($_aryIdTree as $_id) {
            ...
            if ($_index == count($_aryIdTree)) {
                $_out .= '  >  <b>'
                    . $_aryCategoryDataView['category_name'] . '</b>';
            } else {
                $_out .= '  > '
                    . $_aryCategoryDataView['category_name'];
            }
            $_index++;
        }
        return $_out;
    ...
    public function getEnNoaBreadcrumb($idTree) {
        if (!$idTree) {
            return;
        }
    ...
    public function getPrevNext($sort, $categoryId) {

//        <a class="btn" href="">上一篇：新产品S52F增量式实心轴编码器</a>
//        <a class="btn" href="">下一篇：公司新研发国内最小KN35伺服电机编码器</a>

        $_out = '';
        $_prevRs = $this->repoArc->prev($sort, $categoryId);
        if ($_prevRs) {
            $_out .= '<a title="'.$_prevRs->title.'" class="btn"  href="' . UrlHelper::getPageUrl($_prevRs) . '">上一篇：'.$_prevRs->title.'</a>';
        } else {
            $_out .= '<a href="javascript:;" class="btn">上一篇：无</a>';
        }

        $_nextRs = $this->repoArc->next($sort, $categoryId);
        if ($_nextRs) {
            $_out .= '<a title="'.$_nextRs->title.'" class="btn" href="' . UrlHelper::getPageUrl($_nextRs) . '">下一篇：'.$_nextRs->title.'</a>';
        } else {
            $_out .= '<a href="javascript:;" class="btn">下一篇：无</a>';
        }
        return $_out;
    }

    public function getEnPrevNext($sort, $categoryId) {
...
//        <a class="btn" href="">下一篇：公司新研发国内最小KN35伺服电机编码器</a>
        ...
        $_prevRs = $this->repoArc->prev($sort, $categoryId);
        if ($_prevRs) {
            $_out .= '<a title="'.$_prevRs->title.'" class="btn"  href="' . UrlHelper::getPageUrl($_prevRs) . '">The last one：'.$_prevRs->title.'</a>';
        } else {
            $_out .= '<a href="javascript:;" class="btn">The last one：none</a>';
        }

        $_nextRs = $this->repoArc->next($sort, $categoryId);
        if ($_nextRs) {
            $_out .= '<a title="'.$_nextRs->title.'" class="btn" href="' . UrlHelper::getPageUrl($_nextRs) . '">The next one：'.$_nextRs->title.'</a>';
        } else {
            $_out .= '<a href="javascript:;" class="btn">The next one：none</a>';
        }
        return $_out;
    }
